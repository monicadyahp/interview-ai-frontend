    ...
    <section className="w-full px-6 py-[54px]">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-14">
          <h1 className="text-[28px] md:text-[36px] font-extrabold text-black" style={ff}>How It Works ?</h1>
          <p className="mt-3 text-[16px] md:text-[20px] leading-relaxed text-[#222222] max-w-2xl mx-auto" style={ff}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
          {dataHowItWorks.map((data, index) => (
            <div key={data.id} className={index === 0 ? "lg:col-span-2" : ""}>
              <StepCard step={data.step} title={data.title} description={data.description} image={data.image} large={index === 0} />
            </div>
          ))}
        </div>
      </div>
