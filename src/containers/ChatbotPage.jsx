  ...
  return (
    <div className="min-h-screen flex bg-[#F7F7FB]" style={{ fontFamily }}>
      ...
      <aside className="hidden lg:flex w-[240px] bg-white border-r border-[#ECECEC] px-5 py-7 flex-col justify-between shrink-0 fixed top-0 left-0 h-full z-10">
        <div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <SidebarItem imgSrc="/icons/overviewai.png" activeImgSrc="/icons/overviewaiungu.png" label="AI Assistant" active onClick={() => navigate("/chatbot")} />
      ...
      <main className="flex-1 flex flex-col min-w-0 lg:ml-[240px]">
        ...
        {/* Chat Area */}
        <div className="flex-1 flex flex-col px-3 sm:px-6 py-4 sm:py-6 overflow-hidden">

          {/* Chat Card */}
          <div className="flex-1 flex flex-col bg-white rounded-[20px] border border-[#ECECEC] overflow-hidden">

            {/* Chat Header */}
            <div className="flex items-center justify-between gap-2 px-4 sm:px-5 py-4 rounded-t-[20px]"
              style={{ background: "linear-gradient(90deg, #1E1060, #3B2299)" }}>
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-full overflow-hidden border-2 border-white/30 flex-shrink-0">
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-6 space-y-5">
              {!chatStarted ? (
                /* Welcome state */
                <div className="flex flex-col items-center justify-center min-h-full gap-4 sm:gap-5 py-4 sm:py-8">
                  <div>
                    <p className="text-[24px] sm:text-[32px] font-bold text-[#1E1E1E] text-center mb-1" style={{ fontFamily }}>
                      Hi {user?.username?.split(" ")[0] || "Angel"} !
                    </p>
                    <p className="text-[20px] sm:text-[26px] font-bold text-[#1E1E1E] text-center" style={{ fontFamily }}>
                ...
                <>
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.isBot ? "justify-start" : "justify-end"}`}>
                      <div className={`max-w-[85%] sm:max-w-[70%] ${msg.isBot ? "" : ""}`}>
            ...
            {/* Divider */}
            <div className="h-[1px] bg-[#F0F0F0] mx-4 sm:mx-6" />

            {/* Input area */}
            <div className="px-3 sm:px-5 py-3 sm:py-4">
              <form onSubmit={handleSubmit}
                className="flex items-center gap-2 sm:gap-3 border border-[#E5E5E5] rounded-full px-3 sm:px-5 py-2.5 sm:py-3 focus-within:border-[#7B4DFF] transition">
                <button type="button" className="hidden sm:block text-[#999] hover:text-[#7B4DFF] transition flex-shrink-0">
                  <Paperclip size={18} />
                </button>
                <button type="button" className="text-[#999] hover:text-[#7B4DFF] transition flex-shrink-0">
                  <Mic size={18} />
                </button>
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
                  placeholder="Type your questions here..."
                  className="flex-1 min-w-0 outline-none text-[14px] text-[#444] placeholder:text-[#BBB]"
                  style={{ fontFamily }} />
                <button type="submit"
                  className="w-9 h-9 rounded-full flex items-center justify-center text-white hover:opacity-90 transition flex-shrink-0"
