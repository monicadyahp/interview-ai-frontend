...
// FIX: font & icon StatCard digedein
const StatCard = ({ label, value, sub, subColor, iconSrc }) => (
  <div className="bg-white rounded-[20px] border border-[#ECECEC] px-5 py-5 flex items-start justify-between" style={{ fontFamily }}>
    <div>
      {/* FIX: label text-[12px] → text-[13px] */}
      <p className="text-[13px] text-[#999] mb-1">{label}</p>
      {/* FIX: value text-[22px] → text-[26px] */}
    ...
    {/* FIX: icon container w-10 → w-12, icon w-5 → w-7 */}
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen flex bg-[#F7F7FB]" style={{ fontFamily }}>
      ...
      {/* SIDEBAR */}
      <aside className="hidden lg:flex w-[240px] bg-white border-r border-[#ECECEC] px-5 py-7 flex-col justify-between shrink-0">
        <div>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 px-6 py-6 overflow-auto">
          ...
          {/* FIX: Greeting Card — minHeight digedein, teks 2 baris */}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-4">
            ...
            {/* Performance Score Trend — FIX: chart lebih tinggi */}
            <div className="bg-white rounded-[20px] border border-[#ECECEC] p-6">
              <div className="flex items-center justify-between mb-6">
                {/* FIX: title digedein */}
                <p className="text-[17px] font-bold text-[#1E1E1E]">Performance Score Trend</p>
                <div className="flex gap-2">
                  {["6M","1Y"].map((t) => (
                    <button key={t} className="text-[13px] px-4 py-1.5 rounded-full font-semibold bg-[#7B4DFF] text-white">{t}</button>
                  ))}
                </div>
              </div>
              {isLoading ? (
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <LineChart data={chartData}>
                    <XAxis dataKey="label" tick={{ fontSize: 13, fill: "#999" }} axisLine={false} tickLine={false} />
                    <YAxis hide domain={[0, 100]} />
                    <Tooltip contentStyle={{ background: "#fff", border: "1px solid #ECECEC", borderRadius: "12px", fontSize: "13px" }} />
            ...
            {/* Emotions Summary — FIX: donut & text digedein */}
            <div className="bg-white rounded-[20px] border border-[#ECECEC] p-6">
              <p className="text-[17px] font-bold text-[#1E1E1E] mb-5">Emotions Summary</p>
              {/* FIX: donut dari w-[120px] → w-[140px] */}
              <div className="flex justify-center mb-5">
                <div className="relative w-[140px] h-[140px]">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    {/* FIX: angka digedein */}
                    <span className="text-[24px] font-bold text-[#1E1E1E]">{overallScore}%</span>
                    <span className="text-[12px] text-[#999]">Overall</span>
                  </div>
                </div>
              </div>
              {/* FIX: emotion list font digedein */}
                  ...
                  emotionsSummary.map((e) => (
                    <div key={e.label} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[14px] text-[#444]">{e.label}</span>
                      </div>
                      <span className="text-[14px] font-semibold text-[#1E1E1E]">{e.pct}%</span>
                    </div>
                  ))
                ) : (
                  ["Happy","Sad","Normal","Fear","Angry","Disgust"].map((e) => (
                    <div key={e} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[14px] text-[#444]">{e}</span>
                      </div>
