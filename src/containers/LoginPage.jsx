...
const LoginPage = () => {
  ...
  // Dipakai bersama oleh login password & login Google: simpan sesi lalu masuk ke dashboard.
  const handleAuthSuccess = (data) => {
    setUser(data.user);
    localStorage.setItem("token", data.token);
    Swal.fire({
      title: "Selamat Datang!",
      text: `Mari berlatih untuk interview kamu, ${data.user.firstName || "kamu"}!`,
      icon: "success",
      timer: 2000,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        title: "Oops...",
        text: err.response?.data?.message || "Gagal!",
        icon: "error",
      });
    } finally {
      setIsLoading(false);
        ...
        <div className="flex-1 px-8 py-10 overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
            ...
            {isRegister && (
              <div className="flex items-start gap-2">
                <input
                  type="checkbox"
                  className="mt-0.5 accent-[#8039FF]"
                  required
                />
                <p
                  style={{
          ...
          {isRegister && (
            <p
              ...
              style={{ fontFamily, fontSize: "13px", color: "#666666" }}
            >
              Already have an account?{" "}
              <span
                onClick={handleSwitchMode}
                className="text-[#8039FF] cursor-pointer hover:underline font-bold"
              >
                Sign In
              </span>
            </p>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <button
              ...
              onClick={() => loginGoogle()}
              disabled={isLoading}
              className="w-[48px] h-[48px] rounded-full border border-[#E5E5E5] flex items-center justify-center hover:scale-105 transition bg-white disabled:opacity-60 disabled:cursor-not-allowed"
            >
              <img
